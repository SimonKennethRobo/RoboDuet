...
### 2.7 MuJoCo sim2sim 资产

参考 `.refers/LeggedManip_Lab/mujoco/robots/go2_arx5/` 的 ARX5 网格资产，
但**模型本身由 RoboDuet 的训练 URDF 生成**，不是照抄参考模型。原因见下。

生成方式（`scripts/gen_rl_sar_mjcf.py`）：以 rl_sar 自带的 `go2_description/mjcf/go2.xml`
为底座（腿、执行器顺序、传感器块天然符合 rl_sar 约定），把 ARX5 机械臂嫁接上去，
臂的**每一个数字**——连杆位姿、关节轴、关节限位、惯量、安装偏移——
都从 `resources/robots/go2_x5_v3/urdf/go2_x5.urdf` 现场读取。没有手抄。

为什么不直接用参考模型：

- **安装位置不同**。参考模型把臂装在 `pos="0.0 0 0.06"`，RoboDuet 训练用的是
  `xyz="0.05 0.0 0.1"`（前移 5 cm、上移 4 cm）。臂相对机身的质量分布直接影响
  dog policy。
- **连杆坐标系约定不同**。参考模型对 link3/4/5 做了绕 X 轴 180° 的翻转
  （`quat="-3.67e-06 0.99999 0 0"` + 轴取反），与 URDF 等价但表述不同。
  关节角符号其实是一致的，但惯量、子连杆偏移全都写在翻转后的系里，混用必错。
  本模型统一用 URDF 的坐标系，只给 link3/4/5 的**可视网格**加了
  `quat="0 1 0 0"` 来适配参考网格的朝向。
- **臂的版本不同**。参考模型的 joint5/joint6 偏移比 v3 URDF 差 2 mm。

另外把 Go2 腿和机身的惯量也换成了 URDF 的值：rl_sar 自带的是 MuJoCo Menagerie
的 Go2 参数，与 URDF 每个连杆差 40~90 g，四条腿累计 0.5 kg。运动学坐标系完全
一致（逐连杆验证过），所以惯量可以直接替换。

**一处刻意偏离训练：给臂加了 `armature="0.01"`。**
RoboDuet 的 URDF 没声明电机转子惯量，IsaacGym 也不需要——`control_type="M"`
下臂走的是**隐式**位置驱动，无条件稳定。但 rl_sar 是在 200 Hz 上**显式**算 PD
再写力矩，稳定条件是 `kd*dt/I < 2`。用导出的腕部增益（kd = 1~2.5）配裸连杆惯量
（I ≈ 5e-4），这个比值到了 ~10，实测 joint4/5/6 会震荡零点几弧度：

```
             加 armature 前            加 armature 后
  x5_j4      p2p 0.12 rad             p2p 0.0000, 稳态 -0.043 rad（重力下垂）
  x5_j5      p2p 0.57 rad             p2p 0.0000
  x5_j6      p2p 0.76 rad             p2p 0.0000
```

真实的减速箱本来就有这个惯量（rl_sar 自带的 go2 模型每个腿关节也都写着
`armature="0.01"`），所以这是**朝真实靠拢**，不是偏离训练行为。
`damping`/`frictionloss` 保持 0，那两个 URDF 确实没有，而且阻尼由 `rl_kd` 提供。

传感器块顺序（N = 18），与 rl_sar 自带 go2 模型完全一致：
...
[3N+4 .. 3N+6]    gyro
[3N+7 .. 3N+9]    accelerometer
[3N+10 .. 3N+12]  framepos     (世界系)   ← use_base_state_sensor 读这里
[3N+13 .. 3N+15]  framelinvel  (世界系)   ← 读出后转到机体系
```

> 这里修正了先前一版 C++ 的一个错误：当时按 `3N+7 = framelinvel` 写，
> 实际 rl_sar 的 go2 模型在 gyro 之后还有 accelerometer，且 framepos 在
> framelinvel **之前**。已按上表改正。

**验证结果**（`scripts/verify_rl_sar_mjcf.py`，64 组随机关节角）：

```
kinematics: 20 shared bodies, 64 configurations, tolerance 1e-06
  [ok] FL_hip / FL_thigh / FL_calf ... 12 条腿关节   pos ≤ 8.3e-17 m  rot 0
  [ok] x5_link1 .. x5_link8                          pos ≤ 3.3e-16 m  rot 0

mass (vs URDF XML):
  [ok] 26 matched bodies: mjcf 18.491 kg   urdf 18.491 kg

rl_sar layout (num_of_dofs = 18):
  [ok] jointpos / jointvel / jointactuatorfrc occupy 0..53 in hardware order
  [ok] imu_quat 54 / imu_gyro 58 / imu_acc 61 / frame_pos 64 / frame_vel 67
  [ok] 18 actuators in hardware order
```

运动学逐连杆匹配到 1e-16，总质量精确一致，传感器/执行器索引与
`RL_Sim::GetState()`、`SetCommand()` 的位置索引完全对齐。

### 2.8 端到端 sim2sim 结果

C++ 版需要 libtorch / MuJoCo 头文件 / GLFW / TBB 才能编译，本环境没有，
所以另写了 `scripts/sim2sim_mujoco.py`——rl_sar MuJoCo 回路的 Python 镜像，
逐函数对应 `GetState` / `ComputeObservation` / `Forward` / `ComputeOutput` /
`SetCommand`，以及 `GetUp → RLLocomotion` 的状态切换。它加载的是**同一份**
`policy.pt` / `base.yaml` / `config.yaml` / `scene.xml`，观测拼装直接复用
`scripts/rl_sar_obs.py`——也就是 §2.6 里已经与训练逐维比对过的那份代码。

10 秒回合（前 2 秒 GetUp，之后进 RL），结果：

| 指令                 | 机体 vx (m/s)    | 机体 vy (m/s)    | yaw (rad/s)      | 机身高 (m) | upright | 结果     |
| -------------------- | ---------------- | ---------------- | ---------------- | ---------- | ------- | -------- |
| 静止                 | -0.000           | +0.000           | -0.001           | 0.268      | 0.999   | 站住     |
| vx 0.5               | **+0.383** | +0.024           | -0.015           | 0.307      | 0.998   | 走       |
| vx 1.0               | **+0.854** | +0.046           | -0.014           | 0.313      | 0.992   | 走       |
| vx -0.4              | -0.348           | —               | —               | 0.309      | 0.997   | 后退     |
| vy 0.4               | -0.018           | **+0.308** | -0.020           | 0.303      | 0.995   | 侧移     |
| yaw 1.0              | -0.028           | +0.025           | **+0.709** | 0.301      | 0.997   | 原地转   |
| vx 0.4 + yaw 0.6     | +0.302           | +0.047           | +0.390           | 0.302      | 0.997   | 边走边转 |
| vx 0.5, height -0.10 | +0.386           | —               | —               | 0.284      | 0.997   | 走且压低 |

全部保持直立（`upright` = 机体 z 轴在世界 z 上的投影，1.0 为水平），
峰值动作 2.8~6.6，远低于 `clip_actions` 的 10。速度跟踪约为指令的 70~86%，
`body_height` 指令也有响应（0.307 → 0.284）。

**这个结果证明了什么、没证明什么**：证明了导出的模型/配置、关节映射、
观测拼装、动作补零、MJCF 资产、PD 增益这一整条链路是通的。
**没有**证明策略在真机上的表现——手头这个 checkpoint 是 stage-2
（traj_track + dyna_gait）训练出来的 dog policy，被按 stage-1 的方式喂了
全零 `arm_commands`，属于轻微 off-distribution。用真正的 stage-1
checkpoint 重跑一遍，跟踪率应当更好。
...
| 项                                               | 状态                                          |
| ------------------------------------------------ | --------------------------------------------- |
| `src/rl_sar_zoo/go2_x5_description/mjcf/*.xml` | **已完成**，见 §2.7                    |
...
臂的读写和 FAST-LIO 里程计订阅需要你补；写入点是 `RobotState::base`，
坐标系约定见 §2.5。
...
```bash
python scripts/play_by_key_stage1.py --logdir runs/<date>/<run>
# 关掉：--no_rl_sar_export
...
# 默认写到 <logdir>/rl_sar/；--rl_sar_root 可改写到别处
# --robot 可省略：默认从 checkpoint 记录的 asset 路径反查
...
# 3) MJCF 资产：已生成并验证，只有 URDF 变了才需要重跑
python scripts/gen_rl_sar_mjcf.py --zoo /path/to/rl_sar/src/rl_sar_zoo
python scripts/verify_rl_sar_mjcf.py \
    --mjcf /path/to/rl_sar/src/rl_sar_zoo/go2_x5_description/mjcf/go2_x5.xml

# 4) sim2sim —— Python 版（不需要编 C++，只要 mujoco + torch）
python scripts/sim2sim_mujoco.py \
    --robot_dir runs/<date>/<run>/rl_sar/policy/go2_x5 \
    --scene /path/to/rl_sar/src/rl_sar_zoo/go2_x5_description/mjcf/scene.xml \
    --seconds 10 --vx 0.5

# 5) sim2sim —— rl_sar C++ 本体
cd /path/to/rl_sar && ./build.sh          # 会自动拉 libtorch 和 MuJoCo
./cmake_build/bin/rl_sim_mujoco go2_x5 scene
#   0 → 站起,  1 → 进 RL,  9 → 趴下,  P → 被动
#   WSAD/QE 速度,  TG 俯仰,  YH 横滚,  UJ 高度,  空格清零
```

第 4 步是第 5 步的等价 Python 镜像，逐函数对应且共用同一份观测拼装代码，
所以可以先用它把配置/资产调通，再去编 C++。
...
MuJoCo 模型由训练 URDF 生成，运动学逐连杆匹配到 1e-16、总质量精确一致；
整条链路在 MuJoCo 里跑通，站立、前后走、侧移、转向、压低机身全部正常。
**唯一剩余阻塞项是实物节点 `rl_real_go2_x5.cpp`**，需要 X5 机械臂的 SDK 接口细节
和 FAST-LIO 里程计的接线（接入点已经留好：`RobotState::base`）。
