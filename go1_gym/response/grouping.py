...
**Termination asymmetry.**  When one member falls and resets, its phase clock
and command timing are no longer those of the group, so any cross-domain
comparison against it is meaningless.  The group is marked desynchronised and
stays that way until the next group resample re-establishes both.  This is a
correctness requirement, not a refinement: without it the consistency penalty
is largest exactly when it is least meaningful, and the policy is punished for
a fall it has already been punished for.
...
class EnvGrouping:
    ...
    def __init__(
        ...
        self.group_desync = torch.zeros(self.num_groups, dtype=torch.bool, device=device)
    ...
    def mark_desync(self, env_ids: torch.Tensor) -> None:
        """A grouped env reset out of step with its group; invalidate the group.
        ...
        the target altogether -- and one flag per group covers both.
        """
        if env_ids.numel() == 0:
            return
        ...
        if groups.numel() > 0:
            self.group_desync[groups] = True

    def resync(self, group_ids: torch.Tensor) -> None:
        """A group resample re-establishes shared commands and phase."""
        if group_ids.numel() > 0:
            self.group_desync[group_ids] = False
    ...
    def valid(self) -> torch.Tensor:
        ...
        if self.num_groups > 0:
            healthy = self.is_grouped & ~self.group_desync[self.group_of.clamp(min=0)]
