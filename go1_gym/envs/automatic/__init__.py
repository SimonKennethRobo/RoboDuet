...
class VelocityTrackingEasyEnv(LeggedRobot):
    ...
    def get_arm_observations(self):
        ...
        if self.cfg.env.priv_observe_high_freq_goal:
            privileged_obs_buf = torch.cat(
                (privileged_obs_buf, self.obj_pose_in_ee.clone(), self.obj_abg_in_ee.clone()), dim=1
            )
    ...
    def get_dog_observations(self):
        ...
        assert obs_buf.shape[1] == self.cfg.dog.dog_num_observations, (
            f"dog num_observations ({self.cfg.dog.dog_num_observations}) != the number of observations ({obs_buf.shape[1]})"
        )
...
class JoyWrapper(VelocityTrackingEasyEnv):
    """Extends VelocityTrackingEasyEnv with ROS2 /joy gamepad control.

    Axis/button -> command-channel mapping is driven by a YAML file; see
    config/joy_mapping.yaml for the full reference.

    Parameters
    ----------
    sim_device, headless, cfg:
        Passed straight through to VelocityTrackingEasyEnv.
    joy_config_path:
        Optional path to a custom YAML mapping file.  Defaults to
        config/joy_mapping.yaml at the repository root.

    Usage::

        env = JoyWrapper(sim_device="cuda:0", headless=False, cfg=Cfg)
        # or with custom mapping:
        env = JoyWrapper(..., joy_config_path="/path/to/my_mapping.yaml")
    """

    def __init__(self, sim_device, headless, cfg: Cfg = None,
                 joy_config_path: str = None):
        super().__init__(sim_device, headless, cfg=cfg)
        self._joy = JoyController(joy_config_path)
    ...
    def _apply_joy_commands(self):
        # Build mutable Python lists from current tensor state so that
        # JoyController (which has no torch dependency) can update them.
        dog_len = int(self.commands_dog.shape[1])
        arm_len = int(self.commands_arm.shape[1])
        dog = [float(self.commands_dog[0, i]) for i in range(dog_len)]
        arm = [float(self.commands_arm[0, i]) for i in range(arm_len)]

        result = self._joy.resolve_commands(dog, arm)

        if result == "reset":
            self.reset()
            for i in range(min(3, len(dog))):
                dog[i] = 0.0

        # Write back to tensors
        for i in range(min(len(dog), dog_len)):
            self.commands_dog[0, i] = dog[i]
        for i in range(min(len(arm), arm_len)):
            self.commands_arm[0, i] = arm[i]

    def update_arm_commands(self):
        """Mirror of KeyboardWrapper.update_arm_commands."""
        self.commands_arm_obs[0:1, 0] = self.commands_arm[0:1, 0]
        self.commands_arm_obs[0:1, 1] = self.commands_arm[0:1, 1]
        self.commands_arm_obs[0:1, 2] = self.commands_arm[0:1, 2]
        ...
        assert torch.allclose(
            torch.norm(self.obj_quats[0:1], dim=1),
            torch.ones_like(self.obj_quats[0:1]).to(self.device),
            atol=1e-5,
        ), "quats is not unit vector."
        ...
        if self.cfg.use_rot6d:
            r6d = pt3d.matrix_to_rotation_6d(
                pt3d.quaternion_to_matrix(quats[:, [3, 0, 1, 2]])
            )
            self.commands_arm_obs[0:1, 3:9] = r6d.to(self.device)
        else:
            rpy = self.quat_to_angle(self.obj_quats[0:1]).to(self.device)
