...
from go1_gym.response import LatencyBuffer
...
class WBCEnv(LeggedRobot):
    ...
    def _arm_init_buffers_hook(self):
        ...
        # it, which is worse still.  See go1_gym/response/latency.py.
    ...
    def _resample_dog_obs_latency(self, env_ids):
        ...
        if getattr(self.cfg.domain_rand, "randomize_dog_obs_latency", False):
            lo, hi = self._domain_range(
                getattr(self.cfg.domain_rand, "dog_obs_latency_steps_range", [0, 0]),
                "dog_obs_latency",
            )
    ...
    def _arm_nominalize_twins_hook(self, twins):
        ...
        if hasattr(self, "stage1_ee_payload_mass"):
            ...
            self.stage1_ee_payload_com[twins] = 0.0
        if hasattr(self, "dog_obs_latency"):
            # The twin is the response the others are aligned to, so it has to
            # see the world without delay as well as without payload.
            self.dog_obs_latency.delays[twins] = 0
        if hasattr(self, "arm_link_mass_scales"):
    ...
    def _dog_measurement_snapshot(self):
        ...
        """
        arm_slice = slice(self.num_actions_loco, self.num_actions_loco + self.num_actions_arm)
        # The true, undelayed value: rewards, metrics and the R9 identification
        # export read this attribute and must keep seeing ground truth.  Only
        # the copy that reaches the observation vector is delayed.
        # end_effector_state is refreshed unconditionally, ahead of the stage-2
        # switch, and already carries the grasp-point offset -- no extra FK.
    ...
    def _dog_measurements(self):
        ...
        if not latency_on or self.dog_obs_latency.max_steps == 0:
            return snapshot
        if self._dog_latency_step != self.common_step_counter:
            self._dog_latency_step = self.common_step_counter
            self.dog_obs_latency.push(snapshot)
            if torch.any(self.dog_obs_latency_fill):
        ...
        return self.dog_obs_latency.read(jitter_steps=jitter)

    def _response_observation(self, ee_pos_in_base):
        ...
        # ee_pos_in_base arrives already delayed by the sensing latency: it is
        # forward kinematics of the arm encoders, so it is as late as they are.
        # The reference state and (g, l) are not -- they are integrated from
        # the command and from statistics of quantities delayed upstream.
        return torch.cat(
            (
                reference.xi * scale,
    ...
    def get_dog_observations(self):
        ...
        if self.cfg.env.observe_contact_states:
            obs_buf = torch.cat((obs_buf, sensed("contact_states")), dim=1)
