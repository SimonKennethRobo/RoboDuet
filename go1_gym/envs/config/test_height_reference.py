...
def make_env():
    ...
    # The RoboDuet training profile defaults to flat ground (mesh_type='plane',
    # height_reference='world'); trimesh/terrain-relative height is an opt-in
    # a recipe enables explicitly (see configs/coordination_6gpu.json). This
    # geometry test exercises that opt-in path directly.
    env.cfg.terrain.mesh_type = 'trimesh'
    env.cfg.terrain.height_reference = 'terrain'
...
def test_snapshot_semantics_and_partial_updates():
    # height_reference defaults to 'world' (flat-ground training profile); a
    # snapshot that explicitly recorded 'terrain' (an opted-in recipe) must
    # round-trip, and a partial update must not reset it back to the default.
    cfg = build_roboduet_config()
    cfg.terrain.height_reference = 'terrain'
