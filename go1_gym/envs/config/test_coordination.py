...
def test_recipes_resolve_and_snapshot_roundtrip(name):
    ...
    # 112, not the pre-response-model 90: dog.observation_layout_version=2 plus
    # the R7.1 response-model segments (ee_pos_in_base + reference/deviation
    # state) both add fixed-width terms ahead of any per-recipe choice. History
    # is 50, not 30: RESPONSE_OBS_OVERRIDES deliberately wins over the stage-1
    # table's 30 (see wbc.py's ROBODUET_OVERRIDES comment) to give R7.1 a
    # longer window.
    assert cfg.dog.dog_num_observations == 112 and cfg.dog.dog_num_observation_history == 50
...
def test_rpy_reward_matches_combined_positive_and_negative_attitudes():
    # orientation_control (roll) and pitch_control (pitch) are separate reward
    # terms -- see rewards.py's _orientation_error -- so pull both plus the
    # shared helper they call and check their sum, which is the same combined
    # pitch+roll attitude error this test originally checked as one function.
    path = Path(__file__).resolve().parents[1] / "rewards/rewards.py"
    wanted = {"_orientation_error", "_reward_orientation_control", "_reward_pitch_control"}
    nodes = [n for n in ast.walk(ast.parse(path.read_text()))
             if isinstance(n, ast.FunctionDef) and n.name in wanted]
    namespace = {"torch": torch, "quat_from_angle_axis": quat_from_angle_axis,
                 "quat_mul": quat_mul, "quat_rotate_inverse": quat_rotate_inverse}
    exec(compile(ast.Module(body=nodes, type_ignores=[]), str(path), "exec"), namespace)
    Rewards = type("Rewards", (), {n.name: namespace[n.name] for n in nodes})

    def combined_reward(env):
        container = Rewards()
        container.env = env
        return container._reward_orientation_control() + container._reward_pitch_control()
    ...
    # atol loosened from the original single-matrix-composition version's 1e-12:
    # _orientation_error composes quat_roll/quat_pitch through quat_mul rather
    # than one combined rotation matrix, so float32 rounding differs slightly.
    reward = combined_reward(env)
    torch.testing.assert_close(reward, torch.zeros_like(reward), atol=1e-3, rtol=0)
    env.commands_dog[:, 3:5] *= -1
    assert (combined_reward(env) > 0).all()
