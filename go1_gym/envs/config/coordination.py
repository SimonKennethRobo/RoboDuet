...
def configure_experiment(cfg, args):
    ...
    # Coordination command sampling owns the command vector each step (see
    # LeggedRobot._post_physics_step_callback's coordination_commands branch);
    # R5/R6 response grouping and excitation would independently rewrite it
    # out from under the sampler, exactly as for a benchmark scenario cell --
    # see benchmark/dog_policy/evaluation.py's benchmark_env_for_playback.
    if hasattr(cfg, "response"):
